...
createApp({
  ...
  async onPluginsReady(appkit) {
    // appkit's createApp registry infers this accessor as a union
    // (FilesExport | agents registry). Extract the callable files export
    // so it stays callable inside the nested route closures below, where
    // control-flow narrowing wouldn't reach.
    const getVolume = appkit.files as Extract<
      typeof appkit.files,
      (volumeKey: string) => unknown
    >;
    appkit.server.extend((app) => {
      ...
      app.get("/policy/sp", async (_req, res) => {
        const results = await runProbes([
          ["allow_all", "list", () => getVolume("allow_all").list()],
          ["deny_all", "list", () => getVolume("deny_all").list()],
          ["write_only", "list", () => getVolume("write_only").list()],
        ]);
      ...
      app.get("/policy/obo", async (req, res) => {
        const results = await runProbes([
          [
            "public_read",
            "list",
            () => getVolume("public_read").asUser(req).list(),
          ],
          ["deny_all", "list", () => getVolume("deny_all").asUser(req).list()],
          ["sp_only", "list", () => getVolume("sp_only").asUser(req).list()],
        ]);
        res.json({
          identity: "user",
          xForwardedUser: req.header("x-forwarded-user") ?? null,
      ...
      app.post("/api/dashboard/save-view", async (req, res) => {
        ...
        try {
          const volume = getVolume("files").asUser(req);
       ...
       *    rejects with 403.
       * 3. Programmatic `getVolume("obo_demo").asUser(req).list()` runs
      ...
      app.get("/policy/obo-volume", async (req, res) => {
        ...
        const programmatic: ProbeResult[] = await runProbes([
          ["obo_demo", "list", () => getVolume("obo_demo").asUser(req).list()],
        ]);
