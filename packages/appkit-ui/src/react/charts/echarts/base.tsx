...
export function BaseChart({
  ...
  return (
    <ReactECharts
      ...
      // Replace the whole option on every update rather than merging. The option
      // is rebuilt from scratch each render, and a merge cannot *remove*
      // components — e.g. turning the legend off leaves `legend: undefined`,
      // which a merge ignores, so the old legend lingers. Same for series flags
      // like `showSymbol`. Full replacement keeps the chart in sync with props.
      notMerge={true}
      lazyUpdate={true}
    />
  );
