...
export class PersistentStorage implements CacheStorage {
  ...
  private async runMigrations(): Promise<void> {
    try {
      ...
      await this.pool.query(
        `CREATE UNIQUE INDEX IF NOT EXISTS idx_${this.tableName}_key_hash ON ${this.qualifiedTableName} (key_hash);`,
      );

      await this.pool.query(
        `CREATE INDEX IF NOT EXISTS idx_${this.tableName}_expiry ON ${this.qualifiedTableName} (expiry); `,
      );

      await this.pool.query(
        `CREATE INDEX IF NOT EXISTS idx_${this.tableName}_last_accessed ON ${this.qualifiedTableName} (last_accessed); `,
      );

      await this.pool.query(
        `CREATE INDEX IF NOT EXISTS idx_${this.tableName}_byte_size ON ${this.qualifiedTableName} (byte_size); `,
      );
    } catch (error) {
      logger.error(
