  ...
  private wrapWithAsUser<T extends BasePlugin>(plugin: T) {
    ...
    return {
      ...
      asUser: (req: import("express").Request) => {
        ...
        const spExports = (plugin.exports?.() ?? {}) as Record<string, unknown>;
        // Route each function call through the proxy instead of extracting
        // and re-binding. The proxy's get trap wraps calls in
        // runInUserContext(), which sets the AsyncLocalStorage context that
        // RoutingPool and other context-aware code reads.
        const result: Record<string, unknown> = {};
        for (const [key, val] of Object.entries(spExports)) {
          if (typeof val !== "function") {
            result[key] = (userPlugin as any)[key] ?? val;
            continue;
          }
          result[key] = (...args: unknown[]) => {
            const proxyFn = (userPlugin as any)[key];
            return typeof proxyFn === "function"
              ? proxyFn(...args)
              : (val as Function).call(userPlugin, ...args);
          };
        }
        return result;
      },
