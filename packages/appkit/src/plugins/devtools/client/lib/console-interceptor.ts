...
export function interceptConsole(): ConsoleState {
  ...
  if ((window as any).__APPKIT_DEVTOOLS_CONSOLE_PATCHED__) return state;
  (window as any).__APPKIT_DEVTOOLS_CONSOLE_PATCHED__ = true;
  ...
  for (const level of levels) {
    ...
    console[level] = (...args: unknown[]) => {
      ...
      if (message.startsWith("[appkit-devtools]")) {
        original(...args);
        return;
      }
      push({
