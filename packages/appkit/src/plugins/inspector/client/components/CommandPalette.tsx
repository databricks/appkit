  ...
  const toggleStreamExpand = useCallback(async (pluginName: string, streamId: string) => {
    ...
    try {
      const data = await fetch(
        `/api/inspector/stream-events?plugin=${encodeURIComponent(pluginName)}&streamId=${encodeURIComponent(streamId)}`,
    ...
    const poll = async () => {
      try {
        if (state.view === "streams") {
          const data = await fetch("/api/inspector/streams", {
            headers: { [config.sessionHeader]: "" },
          }).then((r) => r.json());
          dispatch({ type: "SET_STREAMS", data });
        } else if (state.view === "queries") {
          const data = await fetch("/api/inspector/queries", {
            headers: { [config.sessionHeader]: "" },
          }).then((r) => r.json());
          dispatch({ type: "SET_QUERIES", data: data.queries || [] });
        } else if (state.view === "performance") {
          const data = await fetch("/api/inspector/performance", {
            headers: { [config.sessionHeader]: "" },
          }).then((r) => r.json());
          dispatch({ type: "SET_PERFORMANCE", data });
        } else if (state.view === "health") {
          const data = await fetch("/api/inspector/health-dashboard", {
            headers: { [config.sessionHeader]: "" },
          }).then((r) => r.json());
  ...
  const commands: Command[] = [
    ...
    },
    {
      ...
      run: async () => {
        ...
        const response = await fetch("/api/inspector/bridge", {
          method: "POST",
          headers: {
