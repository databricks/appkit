...
import { InspectorPlugin, inspector } from "../inspector";
...
describe("InspectorPlugin", () => {
  ...
  test("factory exposes the inspector plugin name", () => {
    ...
    expect(plugin.getBootstrapContributions()).toEqual([
      {
        ...
        html: '<script src="/api/inspector/bootstrap.js" defer></script>',
        position: "body-end",
      },
    ]);
  ...
  test("context endpoint enriches bundle with plugin metadata and recent events", async () => {
    ...
    const eventReq = createMockRequest({
      ...
      headers: {
        "x-appkit-inspector-session": "session-1",
      },
    });
