...
/**
 * Token counts reported by a model serving endpoint for an agent turn.
 *
 * Surfaced by adapters that capture usage (the Databricks adapter requests it
 * via OpenAI-compatible `stream_options.include_usage`) and emitted on the
 * event stream inside an {@link AgentEvent} `metadata` event under `data.usage`.
 * The numbers are per-turn totals, aggregated across every model step in a
 * single `run()` (initial completion + one completion per tool round).
 */
