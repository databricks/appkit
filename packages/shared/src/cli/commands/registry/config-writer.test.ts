...
describe("writeConfig — additive patching", () => {
  ...
  it("writes the target value even when binding + var already exist", () => {
    // Scaffold scenario: databricks.yml already has the sql-warehouse binding
    // and the top-level variable, but no target value assigned. `add` must
    // still persist the resolved value under targets.default.variables.
    const cwd = makeTempDir();
    fs.writeFileSync(
      path.join(cwd, "databricks.yml"),
      [
        "bundle:",
        "  name: app",
        "resources:",
    ...
    const plan = buildConfigPlan([WAREHOUSE], {
      DATABRICKS_WAREHOUSE_ID: "resolved-wh",
    });
    const result = writeConfig(cwd, plan);

    // No names added (binding + var pre-existed) but the file DID change.
    expect(result.databricksYmlChanged).toBe(true);
    const yml = parseDocument(
      fs.readFileSync(path.join(cwd, "databricks.yml"), "utf-8"),
    ).toJSON();
    expect(yml.targets.default.variables.sql_warehouse_id).toBe("resolved-wh");
  });
