import type { RequirementField, ResourceRequirementRow } from "./requirements";
...
function includeInEnv(field: RequirementField): boolean {
  ...
  if (field.localOnly === false) return false;
  return field.origin !== "platform";
}
...
export async function reconcileEnv(
  ...
  for (const need of needs) {
    ...
    if (current !== undefined && current !== "") {
      resolutions.push({ env: need.env, status: "already-set" });
      continue;
    }
