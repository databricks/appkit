...
function migratePatternA(content: string): MigrationResult {
  ...
  const thenInner = content.slice(thenOpenParen + 1, thenCloseParen).trim();
  ...
  const bodyOpenBrace = thenOpenParen + 1 + thenInner.indexOf("{");
  ...
  // Check for .catch() after .then(...)
  const afterThenClose = content.slice(thenCloseParen + 1);
  const catchMatch = afterThenClose.match(/^\s*\)\s*\.catch\s*\(([^)]*)\)/);
  const directCatchMatch = afterThenClose.match(/^\s*\.catch\s*\(([^)]*)\)/);

  let catchSuffix: string;
  let consumeAfterThen: number;

  if (directCatchMatch) {
    catchSuffix = `.catch(${directCatchMatch[1]})`;
    consumeAfterThen = directCatchMatch[0].length;
  } else if (catchMatch) {
    catchSuffix = `.catch(${catchMatch[1]})`;
    consumeAfterThen = catchMatch[0].length;
  } else {
...
function migratePatternB(content: string): MigrationResult {
  ...
  // Find the extend call and start call in the after-createApp region
  const extendPattern = new RegExp(`\\b${varName}\\.server\\.extend\\s*\\(`);
  const startPattern = new RegExp(
    `(?:await\\s+)?${varName}\\.server\\.start\\s*\\(\\s*\\)\\s*;`,
  );

  const extendExec = extendPattern.exec(afterCreateApp);
...
export const customizeCallbackCommand = new Command("customize-callback")
  .description(
    "Migrate createApp usage from autoStart/extend/start pattern to onPluginsReady callback (formerly customize)",
  )
  ...
  .addHelpText(
    ...
    `
Examples:
  $ appkit codemod customize-callback                    # dry-run, auto-detect files
  $ appkit codemod customize-callback --write            # apply changes
  $ appkit codemod customize-callback --path server.ts   # migrate a specific file`,
  )
  .action(runCodemod);
