...
- Covers the most common onboarding case in one step — "same access as the Service Principal"
- No new concepts; extends the existing Add role flow
- Reduces misconfiguration risk (no manual checkbox matching)

## Not recommended (workarounds)

These approaches were considered and rejected — they push complexity to the wrong layer:
...
- **CLI/SDK automation (AppKit-side)** — e.g. `appkit lakebase prepare`. Papers over a platform gap. Application SDKs should not own database permission logic.
