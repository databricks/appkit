...
| [StandardAgent](Class.StandardAgent.md) | Built-in [AgentInterface](Interface.AgentInterface.md) implementation that wraps a LangGraph `createReactAgent` and translates its stream events into Responses API SSE format. Use this as the default agent unless you need a custom implementation for a different LLM SDK. |
