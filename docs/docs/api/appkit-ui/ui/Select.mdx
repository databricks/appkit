...
| `asChild` | `boolean` |  | - | - |
| `size` | `enum` |  | `default` | - |
| `agentId` | `string` |  | - | Stable id the agent uses to pick an option via the `choose` tool. Strongly recommended for selects, since the trigger's text changes with selection. |
