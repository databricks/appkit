...
Before deploying your app, configure it according to your needs.

### `app.yaml` Configuration

The `app.yaml` file configures your app's runtime environment. Here's a basic example:
...
To use the analytics plugin, bind a SQL warehouse:
...
For more advanced configuration options, see the [Databricks Apps Configuration](https://docs.databricks.com/aws/en/dev-tools/databricks-apps/configuration) documentation for details on:
...
See also: [Configuration guide](./configuration.mdx) for environment variable reference.
