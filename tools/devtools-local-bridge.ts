...
const port = Number(process.env.DEVTOOLS_BRIDGE_PORT || "55107");
const logMode = process.env.DEVTOOLS_BRIDGE_LOG_MODE || "both";
...
function printReceived(bundle: unknown, prompt: string) {
  ...
  if (logMode === "summary" || logMode === "both") {
    console.log(
      "[devtools-bridge] received context",
      JSON.stringify(summary, null, 2),
    );
    if (prompt) {
      console.log("[devtools-bridge] prompt length:", prompt.length, "chars");
    }
  }

  if (logMode === "full" || logMode === "both") {
    console.log("[devtools-bridge] full bundle start");
    console.log(JSON.stringify(bundle, null, 2));
    console.log("[devtools-bridge] full bundle end");
    if (prompt) {
      console.log("[devtools-bridge] prompt start");
      console.log(prompt);
      console.log("[devtools-bridge] prompt end");
    }
  }
}
...
server.listen(port, host, () => {
  console.log(
    `[devtools-bridge] listening on http://${host}:${port} (POST /context)`,
  );
  console.log(
