...
// The script is in tools/, so repo root is one level up
...
function toGithubPath(component: ComponentDoc): string {
  ...
  return filePath.split("/appkit/")[1] || filePath;
}
...
**Source:** [\`${relativePath}\`](https://github.com/databricks/appkit/blob/main/${relativePath})
...
function main() {
  ...
  const validComponents = componentDocs.filter((component) => {
    return (
      Boolean(component.displayName) &&
      !lifecycleMethods.includes(component.displayName!)
    );
  });
  ...
  console.log(`\n✅ Generated ${count} component MDX files in ${outputDir}`);
}
